...
class IssuerService:
    ...
    def createIssuer(self,data):
        try:
            ...
            return Response.error(self.requestId, error=r"{} issuer already exist".format(data['name']), responseCode='01')
    ...
    def getOneIssuer(self,pk):
        try:
            if self.data.issuerRepository.IsExists(pk):
                ...
                return Response.success(self.requestId, data=IssuerSerializer(issuer, many=False).data)
            else:
                self.logger.Info(r"Issuer with id {} does not exist, n\ REQUESTID => {}".format(pk,self.requestId))
                return Response.error(self.requestId, message=r"Issuer with id {} does not exist.".format(pk))
    ...
    def updateIssuer(self, data, pk):
        try:
            if self.data.issuerRepository.IsExists(pk):
                ...
                return Response.error(self.requestId, error=r"Issuer with id {} does not exist".format(pk), responseCode="03")
        except BaseException as ex:
    ...
    def deleteIssuer(self, pk):
        try:
            if self.data.issuerRepository.IsExists(pk):
                ...
                return Response.success(self.requestId, message="Issuer was successfully deleted")
            else:
                self.logger.Info(r"Issuer with id --> {} was not found, requestId --> {}".format(pk,self.requestId))
                return Response.error(self.requestId, error="Issuer does not found", responseCode="03")
        except BaseException as ex:
